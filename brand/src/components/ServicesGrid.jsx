...
const services = [
  {
    ...
    gradient: 'from-[#714AF0] to-[#5D42F5]',
    accentColor: '#D4FF00',
  ...
  },
  {
    ...
    gradient: 'from-[#FF3385] to-[#C933C7]',
    accentColor: '#FFF',
    textColor: 'text-white',
    iconBg: 'bg-white/15',
    icon: (
  ...
  },
  {
    ...
    gradient: 'from-[#4D66FF] to-[#714AF0]',
    accentColor: '#D4FF00',
    textColor: 'text-white',
    iconBg: 'bg-white/15',
    icon: (
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-20">
          ...
          <h2 className="text-5xl md:text-7xl font-black tracking-tight leading-[1.05] uppercase text-[#111] mb-6">
            Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#5D42F5] via-[#C933C7] to-[#F1556C]">Services</span>
          </h2>
          ...
          {services.map((service) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative bg-gradient-to-br ${service.gradient} rounded-[24px] p-8 md:p-10 overflow-hidden transition-all duration-500 ${isHovered ? 'shadow-2xl scale-[1.02]' : 'shadow-lg'}`}
              >
                {/* Large Background Number */}
                <span className="absolute -right-4 -top-6 text-[8rem] font-black leading-none text-white/[0.07] select-none pointer-events-none transition-transform duration-700 group-hover:translate-x-2 group-hover:-translate-y-2">
                  {service.id}
                </span>

                {/* Icon */}
                <div className={`${service.iconBg} w-14 h-14 rounded-2xl flex items-center justify-center mb-6 ${service.textColor} backdrop-blur-sm transition-transform duration-300 group-hover:scale-110`}>
                  {service.icon}
                </div>
                ...
                <p className="text-sm font-bold mb-4 opacity-60" style={{ color: service.accentColor }}>
                  {service.tagline}
                </p>

                {/* Description */}
                <p className={`${service.textColor} opacity-80 text-sm leading-relaxed mb-8`}>
                  {service.description}
                </p>
                ...
                <div className="flex flex-wrap gap-2 mb-8">
                  {service.deliverables.map((item, i) => (
                    <span
                      key={i}
                      className="text-[11px] font-bold tracking-wider uppercase px-3 py-1.5 rounded-full bg-white/10 text-white/90 backdrop-blur-sm"
                    >
                      {item}
                    </span>
                  ))}
                ...
                {/* CTA Arrow */}
                <div className={`flex items-center gap-2 ${service.textColor} opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0`}>
